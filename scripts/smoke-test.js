...
const { getDefaultRulesetTemplate } = require('../out/default-ruleset-template');
...
(async () => {
  const rulesXml = await getDefaultRulesetTemplate(workspace);
  ...
  for (const requiredRule of newRules) {
    if (!rulesXml.includes(requiredRule)) {
      console.error(`Smoke test failed: default ruleset template is missing ${requiredRule}.`);
      process.exit(1);
    }
  }
